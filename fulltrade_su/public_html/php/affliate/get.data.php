...
$deposit = R::findAll('pay');
$withdraw = R::findAll('withdraw');
...
foreach ($withdraw as $key => $value) {
    $userDataDepoit = R::findOne('users', 'id = ?', [$value->user]);
	if ($userData['refcode'] == $userDataDepoit['refer']) {
	    if($value->status == 1){
	        $dayUser = date('d', $value->date);
    		$weekUser = date('W', $value->date);
    		$monthUser = date('m', $value->date);
    		...
    		if ($dayUser == $day) {
    			$dayWithdraw += $value->sum;
    		}
    
    		if ($weekUser == $week) {
    			$weekWithdraw += $value->sum;
    		}
    
    		if ($monthUser == $month) {
    			$monthWithdraw += $value->sum;
    		}
    
    		$allWithdraw += $value->sum;
        }
	}
}
...
foreach ($deposit as $key => $value) {
	...
	if ($userData['refcode'] == $userDataDepoit['refer']) {
	    if($value->status == 1){
	        $dayUser = date('d', $value->payid);
    		$weekUser = date('W', $value->payid);
    		$monthUser = date('m', $value->payid);
    
    		if ($dayUser == $day) {
    			$dayDeposits += $value->sum;
    		}
    
    		if ($weekUser == $week) {
    			$weekDeposits += $value->sum;
    		}
    
    		if ($monthUser == $month) {
    			$monthDeposits += $value->sum;
    		}
    
    		$allDeposits += $value->sum;
        }
	}
}
